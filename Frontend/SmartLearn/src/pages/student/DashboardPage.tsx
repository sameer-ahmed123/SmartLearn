  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const assignRes = await apiClient.get("/assessments/student-assignments/");
        ...
        setLectureChartData([
          ...
          { name: 'Lectures', value: m?.completed_lectures || 0, fill: '#f59e0b' },
      ...
      <main className={styles.mainContainer}>
        ...
        <div className={styles.welcomeBanner}>
          <div className={styles.bannerLeft}>
              <h2 className={styles.bannerTitle}>Keep it up <span className={styles.highlight}>{metrics?.full_name?.split(' ')[0] || 'Student'}!</span></h2>
        ...
        <div className={styles.statsRow}>
          {[
            ...
            { label: 'LECTURES', val: metrics?.completed_lectures || 0, icon: <PlayCircle />, color: '#10b981' },
          ...
          <div className={styles.card}>
            ...
            <div className={styles.calendarGrid}>
              {['S','M','T','W','T','F','S'].map((day, index) => (
                <span key={`header-${index}`} className={styles.calHead}>{day}</span>
              ))}
              {renderCalendar()}
            </div>
          </div>
                    ...
                    metrics.course_progress.map((course: any, idx: number) => (
                      <div key={idx} style={{ marginBottom: '15px' }}>
                        ...
                        <div className={styles.progBar}>
                          <div style={{ width: `${course.progress}%`, background: idx % 2 === 0 ? '#4f46e5' : '#10b981' }}></div>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p style={{ fontSize: '12px', color: '#64748b' }}>No courses enrolled yet.</p>
                  )}
              </div>
            </div>
            <div className={`${styles.card} ${styles.centerText}`}>
              ...
              <div className={styles.circleContainer}>
                  <svg viewBox="0 0 36 36" className={styles.circularChart}>
                    ...
                    <path className={styles.circle} style={{stroke: '#10b981'}} strokeDasharray="75, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                  </svg>
