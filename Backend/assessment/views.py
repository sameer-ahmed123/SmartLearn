...
def teacher_analytics(request):
    ...
    for course in courses:
        score = QuizSubmission.objects.filter(
            quiz__lecture__content_source__course=course).aggregate(avg=Avg('score'))['avg'] or 0
        course_list.append({
            "name": course.title,
    ...
    students = User.objects.filter(
        course_enrollments__course__teacher=teacher).distinct()[:5]
    student_progress = []
    colors = ['#6366f1', '#10b981', '#f59e0b', '#f43f5e', '#8b5cf6']

    for idx, student in enumerate(students):
        prog = QuizSubmission.objects.filter(
            user=student, quiz__lecture__content_source__course__teacher=teacher).aggregate(avg=Avg('score'))['avg'] or 0
...
def teacher_assignment_list(request):
    user = request.user
    assignments = Assignment.objects.filter(lecture__content_source__course__teacher=user).select_related(
...
def teacher_quiz_list(request):
    user = request.user
    quizzes = Quiz.objects.filter(lecture__content_source__course__teacher=user).select_related(
        'lecture__content_source__course', 'lecture')
    quiz_list = []
...
def teacher_gradebook_summary(request):
    ...
    for student in students:
        asg_filter = {
            'user': student, 'assignment__lecture__content_source__course__teacher': teacher, 'score__isnull': False}
        ...
        asg_avg = AssignmentSubmission.objects.filter(
            **asg_filter).aggregate(avg=Avg('score'))['avg'] or 0
        quiz_avg = QuizSubmission.objects.filter(
            **quiz_filter).aggregate(avg=Avg('score'))['avg'] or 0
        exam_marks = 0
...
def student_gradebook_summary(request):
    user = request.user
    assignments = AssignmentSubmission.objects.filter(
        user=user).select_related('assignment__lecture__content_source__course')
    ...
    for q in quizzes:
        ...
        if c_id not in course_data:
            course_data[c_id] = {"course": course.title, "instructor": course.teacher.full_name or course.teacher.username, "asg_scores": [
            ], "quiz_scores": [], "exam_marks": 0, "status": "Completed"}
    ...
    for c_id, data in course_data.items():
        asg_avg = sum(data["asg_scores"]) / \
            len(data["asg_scores"]) if data["asg_scores"] else 0
        quiz_avg = sum(data["quiz_scores"]) / \
            len(data["quiz_scores"]) if data["quiz_scores"] else 0
        ...
        final_course_list.append({"course": data["course"], "instructor": data["instructor"], "assignments_marks": round(asg_avg, 2), "quizzes_marks": round(
            quiz_avg, 2), "exam_marks": data["exam_marks"], "score": total_score, "grade": grade, "status": data["status"]})

    course_count = len(final_course_list)
    gpa = round((total_avg_sum / (course_count * 25)),
                2) if course_count > 0 else 0.00
...
def student_analytics(request):
    ...
    total_asg_count = Assignment.objects.filter(
        lecture__content_source__course__enrollments__student=user, status='published').count()
    ...
    for enrollment in enrolled_courses:
        ...
        course_quiz_avg = QuizSubmission.objects.filter(
            user=user, quiz__lecture__content_source__course=course).aggregate(avg=Avg('score'))['avg'] or 0
        watch_progress = 70 if course_quiz_avg > 50 else 30
        courses_data.append(
            {"name": course.title, "watch": watch_progress, "quiz": round(course_quiz_avg, 1)})

    active_assignments = []
    all_asg = Assignment.objects.filter(
        lecture__content_source__course__enrollments__student=user, status='published').order_by('-deadline')[:3]
    for asg in all_asg:
        sub = assignments.filter(assignment=asg).first()
        progress = 100 if sub else 0
        deadline_label = "Submitted" if sub else (
            asg.deadline.strftime("%d %b") if asg.deadline else "Active")
        if asg.deadline and timezone.now() > asg.deadline and not sub:
