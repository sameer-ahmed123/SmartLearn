...
def generate_lecture_from_source(content_source_id):
    ...
    try:
        ...
        if isinstance(file_data, str) and file_data.startswith("Error"):
            logger.error(f"Failed to process file for lecture ID {content_source_id}: {file_data}")
        ...
        # Generate Script and Context
        generated_script, context_text = generate_lecture_script(ai_prompt, file_data)
        
        # --- STEP 2: Generate Video and Transcript (External API Call) ---
        
        # Generate Video and Transcripts (Takes in the Generated Script)
